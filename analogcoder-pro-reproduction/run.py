...
parser.add_argument('--model', type=str, default=os.environ.get("BAILIAN_CODE_MODEL", "qwen3-coder-plus"))
parser.add_argument('--vlm-model', type=str, default=os.environ.get("BAILIAN_VLM_MODEL", "qwen3.7-plus"))
...
parser.add_argument("--api-retry-delay", type=float, default=5.0)
...
args.base_url = (args.base_url or os.environ.get("BAILIAN_BASE_URL") or
                 os.environ.get("OPENAI_BASE_URL") or
                 "https://dashscope.aliyuncs.com/compatible-mode/v1")
if not args.api_key:
    parser.error("Missing API key. Set DASHSCOPE_API_KEY (recommended) or pass --api_key.")
if args.api_max_retries < 1:
    parser.error("--api-max-retries must be at least 1")
if args.vlm_timeout <= 0:
    parser.error("--vlm-timeout must be positive")
...
client = OpenAI(api_key=args.api_key, base_url=args.base_url, max_retries=0, timeout=60.0)
...
if not args.no_vlm:
    client_vlm = OpenAI(
        ...
        max_retries=0, timeout=args.vlm_timeout
    )
...
def subprocess_env():
    ...
    env["PYTHONUTF8"] = "1"
    matplotlib_config = REPO_ROOT / ".runtime" / "matplotlib"
...
def request_completion(api_client, model, messages, temperature, label):
    """Call an OpenAI-compatible endpoint with a finite, recorded retry policy."""
    last_error = None
    attempts_made = 0
    for attempt in range(1, args.api_max_retries + 1):
        ...
        try:
            ...
            valid = (completion is not None and not isinstance(completion, str) and
                     getattr(completion, "choices", None) and
                     completion.choices[0].message.content is not None)
            if not valid:
                raise RuntimeError("API returned an empty or malformed completion")
            usage = getattr(completion, "usage", None)
            event = {
                "label": label, "model": model, "attempt": attempt,
                "status": "success", "elapsed_seconds": round(time.time() - started, 3)
            }
            if usage:
                event["tokens"] = {
            ...
            API_EVENTS.append({
                ...
                "error": str(exc), "elapsed_seconds": round(time.time() - started, 3)
            })
            print(f"{label} API attempt {attempt}/{args.api_max_retries} failed: {exc}")
            if getattr(exc, "status_code", None) in (401, 403):
                print(f"{label} authentication/authorization error is not retryable.")
                break
            if attempt < args.api_max_retries:
                time.sleep(args.api_retry_delay)
    raise RuntimeError(
        f"{label} API request failed after {attempts_made} attempt(s)"
    ) from last_error
...
def extract_code(generated_content):
    ...
    if not args.ngspice:
        ...
        if "configure_pyspice()" not in code:
            code = ("from pyspice_runtime import configure_pyspice\n"
                    "configure_pyspice()\n" + code)
...
def run_code(file):
    ...
    floating_node = ""
    try:
        result = subprocess.run([sys.executable, "-u", file], check=True, text=True,
            stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=60, env=subprocess_env())
...
def check_function(task_id, code_path, task_type):
    ...
    if task_type == "CurrentMirror":
        ...
        return 0, ""
    try:
        result = subprocess.run([sys.executable, "-u", fwrite_code_path], check=True, text=True,
                stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=subprocess_env())
        func_error = 0
        return_message = ""
    except subprocess.CalledProcessError as e:
        func_error = 1
...
def optimize_code(code_path, task_id, task_type, task):
    ...
    if empty_code_error:
        raise RuntimeError("Optimization response did not contain executable code")
    ...
    subprocess.run(
        [sys.executable, "circuit_optimizer_helper.py", answer_code_name, target],
        cwd=base_path, check=True, env=subprocess_env()
    )
...
def work(task, input, output, task_id, it, background, task_type, flog = None,
    ...
    completion = request_completion(
        client, args.model, messages, args.temperature, "initial_generation"
    )
    ...
    input_log_path = '{}/p{}/p{}_{}_input.txt'.format(model_dir, task_id, task_id, it)
    output_log_path = '{}/p{}/p{}_{}_output.txt'.format(model_dir, task_id, task_id, it)
    fwrite_input = open(input_log_path, 'w')
    fwrite_input.write(prompt)
    fwrite_input.flush()
    fwrite_output = open(output_log_path, 'w')
    ...
    while code_id < args.num_of_retry:
        ...
        if execution_error == 0 and simulation_error == 0:
            if task_type not in complex_task_type:
                ...
                result = subprocess.run([sys.executable, "-u", code_netlist_path], check=True, text=True,
                    stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=subprocess_env())
                ...
                if "Opamp" in task_type or "Amplifier" in task_type:
                    ...
                    try:
                        subprocess.run([sys.executable, "-u", dc_sweep_code_path], check=True, text=True,
                            stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=subprocess_env())
                ...
                if warning == 0:
                    ...
                    func_error_message = func_error_message.replace("Unsupported Ngspice version 46", "")
                    if func_error ==0:
                ...
                if "import math" not in code:
                    code = "import math\n" + code
                op_code_path = code_path.rsplit(".", 1)[0] + "_op.py"
                shutil.copy(code_path, op_code_path)
                print("copy file {} to {}".format(code_path, op_code_path))
                with open(f"{code_path}", "w") as f:
                    f.write(code)
                execution_error, simulation_error, execution_error_info, floating_node = run_code(code_path)
                if execution_error == 0 and simulation_error == 0:
                    ...
                    with open(token_info_path, 'w') as ftmp_write:
                        ...
                        ftmp_write.write(f"Total completion tokens: {total_completion_tokens}\n")
                        ftmp_write.write(f"Status: Success\n")
                    schematic_dir = Path(code_path).parent / f"schematic_retry{code_id}"
                    schematic_log = schematic_dir / "export_stdout.txt"
                    schematic_error_log = schematic_dir / "export_stderr.txt"
                    schematic_dir.mkdir(parents=True, exist_ok=True)
                    try:
                        schematic_result = subprocess.run(
                            [sys.executable, str(REPO_ROOT / "repro_tools" / "schematic" /
                             "export_from_pyspice.py"), op_code_path,
                             "--output-dir", str(schematic_dir)],
                            check=True, text=True, stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE, timeout=120, env=subprocess_env()
                        )
                        schematic_log.write_text(schematic_result.stdout, encoding="utf-8")
                        schematic_error_log.write_text(schematic_result.stderr, encoding="utf-8")
                    except Exception as schematic_error:
                        (schematic_dir / "schematic_export_error.txt").write_text(
                            f"{type(schematic_error).__name__}: {schematic_error}",
                            encoding="utf-8"
                        )
        ...
        execution_error_info = execution_error_info.replace("Unsupported Ngspice version 46", "")
        ...
        if dc_sweep_error == 1:
            ...
            if empty_code_error == 1:
                ...
                if os.path.exists(figure_path) and client_vlm is not None and code_id < args.num_of_retry - 1:
                    ...
                    try:
                        completion_vlm = request_completion(
                            client_vlm, vlm_model, messages_vlm, 0.0,
                            f"vlm_debug_retry_{code_id}"
                        )
                        answer_vlm = completion_vlm.choices[0].message.content
                        if hasattr(completion_vlm, 'usage') and completion_vlm.usage:
                            total_tokens += completion_vlm.usage.total_tokens or 0
                            total_prompt_tokens += completion_vlm.usage.prompt_tokens or 0
                            total_completion_tokens += completion_vlm.usage.completion_tokens or 0
                    except Exception as vlm_error:
                        answer_vlm = (
                            "VLM analysis unavailable after finite retries. "
                            "Continuing with the simulator error only.\n"
                            f"{type(vlm_error).__name__}: {vlm_error}"
                        )
                        vlm_error_path = "{}/p{}/{}/p{}_{}_{}_vlm_error.txt".format(
                            model_dir, task_id, it, task_id, it, code_id
                        )
                        with open(vlm_error_path, "w") as f:
                            f.write(answer_vlm)
        ...
        completion = request_completion(
            client, args.model, messages, args.temperature,
            f"repair_generation_{code_id}"
        )
    ...
    run_dir = Path(model_dir) / f"p{task_id}" / str(it)
    messages_path = run_dir / f"p{task_id}_{it}_messages.json"
    with open(messages_path, 'w', encoding='utf-8') as fwrite:
        json.dump(messages, fwrite, ensure_ascii=False, indent=2)
    fwrite_input.close()
    fwrite_output.close()
    shutil.copy2(input_log_path, run_dir / "prompt_history.txt")
    shutil.copy2(output_log_path, run_dir / "answer_history.md")
    ...
    success_files = sorted(str(path.relative_to(run_dir)) for path in run_dir.glob("*_success.py"))
    ...
    run_record = {
        ...
        "task_type": task_type,
        "iteration": int(it),
        "status": "success" if success_files else "failed",
        "code_model": args.model,
        "vlm_model": None if args.no_vlm else vlm_model,
        "base_url": args.base_url,
        "temperature": args.temperature,
        "max_design_attempts": args.num_of_retry,
        ...
        "api_events": API_EVENTS[event_start:],
        ...
        "environment": {
            ...
            "ngspice_dll": str(os.environ.get("NGSPICE_DLL_PATH", "vendored-ngspice-46")),
        },
    ...
    with open(run_dir / "run_record.json", "w", encoding="utf-8") as record_file:
...
def get_retrieval(task, task_id):
    ...
    if args.retrieval:
        completion = request_completion(
            client, args.model, messages, args.temperature, "retrieval"
        )
        answer = completion.choices[0].message.content
...
if __name__ == "__main__":
    try:
        ...
        failure_dir = REPO_ROOT / "run_artifacts" / "orchestrator_failures"
        ...
        failure_record = {
            ...
            "counted_as_independent_run": False,
        }
        failure_path = failure_dir / (
            f"task_{args.task_id}_{timestamp.strftime('%Y%m%dT%H%M%SZ')}.json"
        )
        with open(failure_path, "w", encoding="utf-8") as failure_file:
            json.dump(failure_record, failure_file, ensure_ascii=False, indent=2)
        print(f"Failure record saved to: {failure_path}", file=sys.stderr)
        raise
