...
circuit.V('rfp', 'Vrfp', circuit.gnd, 2.5)  # RF+ input
circuit.V('rfn', 'Vrfn', circuit.gnd, 2.5)  # RF- input
circuit.V('lop', 'Vlop', circuit.gnd, 4.0)  # LO+ input (high state)
circuit.V('lon', 'Vlon', circuit.gnd, 1.0)  # LO- input (low state)
circuit.V('bias_tail', 'Vbias_tail', circuit.gnd, 1.0)  # Bias voltage for tail current source
circuit.V('bias_load', 'Vbias_load', circuit.gnd, 4.0)  # Bias voltage for active loads
# CORRECTED STANDARD GILBERT CELL TOPOLOGY:
# 1. Top-level RF differential pair with tail current
# M1: RF+ input transistor
circuit.MOSFET('1', 'Node_A', 'Vrfp', 'Node_X', 'Node_X', model='nmos_model', w=50e-6, l=1e-6)
# M2: RF- input transistor
circuit.MOSFET('2', 'Node_B', 'Vrfn', 'Node_Y', 'Node_Y', model='nmos_model', w=50e-6, l=1e-6)
# 2. LO switching quad (switches current between upper and lower branches)
# M3: Switch from Node_X to ground, controlled by Vlop
circuit.MOSFET('3', 'Node_X', 'Vlop', circuit.gnd, circuit.gnd, model='nmos_model', w=50e-6, l=1e-6)
# M4: Switch from Node_X to ground, controlled by Vlon
circuit.MOSFET('4', 'Node_X', 'Vlon', circuit.gnd, circuit.gnd, model='nmos_model', w=50e-6, l=1e-6)
# M5: Switch from Node_Y to ground, controlled by Vlon
circuit.MOSFET('5', 'Node_Y', 'Vlon', circuit.gnd, circuit.gnd, model='nmos_model', w=50e-6, l=1e-6)
# M6: Switch from Node_Y to ground, controlled by Vlop
circuit.MOSFET('6', 'Node_Y', 'Vlop', circuit.gnd, circuit.gnd, model='nmos_model', w=50e-6, l=1e-6)
# 3. Tail current source
# M9: Tail current source connecting Node_X and Node_Y to ground
circuit.MOSFET('9', 'Node_XY_common', 'Vbias_tail', circuit.gnd, circuit.gnd, model='nmos_model', w=20e-6, l=1e-6)
# Connect both Node_X and Node_Y to the tail current source
circuit.R('tail1', 'Node_X', 'Node_XY_common', 0.1@u_Ω)  # Very small resistance to connect nodes
circuit.R('tail2', 'Node_Y', 'Node_XY_common', 0.1@u_Ω)  # Very small resistance to connect nodes
# 4. Active loads (PMOS) - Pulling outputs to Vdd
# M7: Active load for positive output, connected to Node_A
circuit.MOSFET('7', 'Voutp', 'Vbias_load', 'Vdd', 'Vdd', model='pmos_model', w=100e-6, l=1e-6)
# M8: Active load for negative output, connected to Node_B
circuit.MOSFET('8', 'Voutn', 'Vbias_load', 'Vdd', 'Vdd', model='pmos_model', w=100e-6, l=1e-6)
# 5. Connect the RF outputs to the final outputs
circuit.R('load1', 'Voutp', 'Node_A', 1@u_kΩ)  # Load resistor for output
circuit.R('load2', 'Voutn', 'Node_B', 1@u_kΩ)  # Load resistor for output
# Analysis Part
...
plt.tight_layout()
plt.savefig('qwen3-coder-plus_retry_4/p19/2/p19_2_3_figure.png')
# plt.show()
