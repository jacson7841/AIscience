...
circuit.V('rfp', 'Vrfp', circuit.gnd, 2.5)  # RF positive input (at threshold + overdrive)
circuit.V('rfn', 'Vrfn', circuit.gnd, 2.5)  # RF negative input (at threshold + overdrive)
circuit.V('lop', 'Vlop', circuit.gnd, 4.5)  # LO positive input (high state for PMOS switching)
circuit.V('lon', 'Vlon', circuit.gnd, 0.5)  # LO negative input (low state for PMOS switching)
# Tail current source for RF differential pair (connected to Vdd via NMOS current source)
# Using M9 as the tail current source NMOS
circuit.MOSFET('9', 'TailNode', 'Vbias_tail', circuit.gnd, circuit.gnd, model='nmos_model', w=20e-6, l=1e-6)
circuit.V('bias_tail', 'Vbias_tail', circuit.gnd, 1.0)  # Bias for tail current source
# RF Differential Pair (M1, M2)
circuit.MOSFET('1', 'Drain1', 'Vrfp', 'TailNode', 'TailNode', model='nmos_model', w=50e-6, l=1e-6)
circuit.MOSFET('2', 'Drain2', 'Vrfn', 'TailNode', 'TailNode', model='nmos_model', w=50e-6, l=1e-6)
# LO Switching Quad (M3, M4, M5, M6)
# These transistors route currents from RF pair to output loads based on LO signals
circuit.MOSFET('3', 'OutPNode', 'Vlop', 'Drain1', 'Drain1', model='nmos_model', w=50e-6, l=1e-6)  # Connects M1 to OutP
circuit.MOSFET('4', 'OutPNode', 'Vlon', 'Drain2', 'Drain2', model='nmos_model', w=50e-6, l=1e-6)  # Connects M2 to OutP
circuit.MOSFET('5', 'OutNNode', 'Vlop', 'Drain2', 'Drain2', model='nmos_model', w=50e-6, l=1e-6)  # Connects M2 to OutN
circuit.MOSFET('6', 'OutNNode', 'Vlon', 'Drain1', 'Drain1', model='nmos_model', w=50e-6, l=1e-6)  # Connects M1 to OutN
# Output Load Current Sources (M7, M8 as PMOS active loads)
# These connect from Vdd to the output nodes to create proper current paths
circuit.MOSFET('7', 'Voutp', 'Vbias_load', 'Vdd', 'Vdd', model='pmos_model', w=100e-6, l=1e-6)  # Load for Voutp
circuit.MOSFET('8', 'Voutn', 'Vbias_load', 'Vdd', 'Vdd', model='pmos_model', w=100e-6, l=1e-6)  # Load for Voutn
circuit.V('bias_load', 'Vbias_load', circuit.gnd, 1.0)  # Bias for load current sources
# Direct connections from switching quad outputs to final outputs
# These establish the current paths from switching transistors to output nodes
circuit.R('load_p', 'OutPNode', 'Voutp', 1@u_kΩ)  # Load resistor for output P
circuit.R('load_n', 'OutNNode', 'Voutn', 1@u_kΩ)  # Load resistor for output N
...
plt.tight_layout()
plt.savefig('qwen3-coder-plus_retry_4/p19/1/p19_1_1_figure.png')
# plt.show()
